<template>
  <div id="edit-demo" class="demo">
    <div class="viewport" @click="select(null)" @mousedown.capture="blockEvents" @wheel.capture="blockEvents">
      <screen ref="screen">
        <g v-for="edge in graph.edges" :key="edge.id" @click.stop="select(edge)">
          <edge
            :class="selection && selection.id === edge.id && 'selected'"
            :data="edge"
            :nodes="graph.nodes"
          >
          </edge>
        ...
        <g v-for="node in graph.nodes" :key="node.id">
          <node ref="node" :data="node" :class="isSelected(node) && 'selected'" :text-select="node.textSelect" :use-drag="node.useDrag">
            <div @click.stop="select(node)" v-html="node.html">
            </div>
          </node>
        </g>
      </screen>
